<?php
if ( ! class_exists('Seminar_Bulgarian_Registrants') ) {

    class Seminar_Bulgarian_Registrants {

        private $reg_year;
        private $table;
        private $page_slug = 'seminar-bulgarian-registrants';

        public function __construct() {
            global $wpdb;
            $this->table = $wpdb->prefix . 'view_bulgarian_registrants';
            ...
            add_action( 'admin_post_sr_export_bulgarian_registrants', array( $this, 'handle_export_bulgarian_registrants') );
        }
        ...
        public function render_page() {
            $display_rows = array();
            if ( isset( $_POST['view-bulgarian-registrants'] ) ) {
                ...
                if ( ! isset( $_POST['sr_bulgarian_registrants_nonce'] ) || ! wp_verify_nonce( $_POST['sr_bulgarian_registrants_nonce'], 'sr_bulgarian_registrants' ) ) {
                    echo '<div class="wrap"><p>Invalid request.</p></div>';
                    return;
                }

                $rows = $this->getBulgarianRegistrants();
                if ( ! empty( $rows ) ) {
            ...
            $template = SR_PLUGIN_DIR . '/templates/bulgarian-registrants.php';
            if ( file_exists( $template ) ) {
                $reg_year = $this->reg_year;
                include $template;
            } else {
                echo '<div class="wrap"><h1>Bulgarian Registrants</h1>';
                echo '<p>Template file missing: ' . esc_html( $template ) . '</p></div>';
            }
        }

        public function handle_export_bulgarian_registrants() {
            if ( ! current_user_can( 'edit_users' ) ) {
                wp_die( 'Insufficient permissions.' );
            }

            check_admin_referer( 'sr_bulgarian_registrants', 'sr_bulgarian_registrants_nonce' );
        ...
        private function outputCsv( $rows ) {
            ...
            $filename = 'bulgarian-registrants-' . $this->reg_year . '.csv';
        ...
        private function getBulgarianRegistrants() {
            global $wpdb;
            if ( ! class_exists( 'Seminar_Registration_Queries' ) ) {
                return array();
            }
            $sql_template = Seminar_Registration_Queries::get_bulgarian_registrants_sql();
...
if ( class_exists( 'Seminar_Bulgarian_Registrants' ) && ! isset( $seminar_bulgarian_registrants ) ) {
    $seminar_bulgarian_registrants = new Seminar_Bulgarian_Registrants();
}
