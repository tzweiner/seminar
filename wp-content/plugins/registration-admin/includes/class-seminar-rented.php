<?php
if ( ! class_exists('Seminar_Rented') ) {

    class Seminar_Rented {

        private $reg_year;
        private $table;
        private $page_slug = 'seminar-rented';

        public function __construct() {
            global $wpdb;
            $this->table = $wpdb->prefix . 'view_rented';
            ...
            add_action( 'admin_post_sr_export_rented', array( $this, 'handle_export_rented' ) );
        }
...
//            );
        ...
        public function enqueue_assets( $hook ) {
            if ( isset( $_GET['page'] ) && $_GET['page'] === $this->page_slug ) {
                if ( defined( 'SR_ASSETS_URL' ) ) {
                    wp_register_style( 'sr-admin-rented', SR_ASSETS_URL . '/css/common.css', array(), '1.1' );
                    wp_enqueue_style( 'sr-admin-rented' );
                }
            }
        }
        ...
        public function render_page() {
            ...
            if ( isset( $_POST['view-rented'] ) ) {
                ...
                if ( ! isset( $_POST['sr_rented_nonce'] ) || ! wp_verify_nonce( $_POST['sr_rented_nonce'], 'sr_rented' ) ) {
            ...
            $template = SR_PLUGIN_DIR . '/templates/rented.php';
            if ( file_exists( $template ) ) {
                $reg_year = $this->reg_year;
                include $template;
            } else {
                echo '<div class="wrap"><h1>Registrants with rentals</h1>';
                echo '<p>Template file missing: ' . esc_html( $template ) . '</p></div>';
            }
        }

        public function handle_export_rented() {
            if ( ! current_user_can( 'edit_users' ) ) {
                wp_die( 'Insufficient permissions.' );
            }

            check_admin_referer( 'sr_rented', 'sr_rented_nonce' );
        ...
        private function parse_registrant_for_display( $registrant ) {
            ...
            $class_name    = isset( $registrant->class_name ) ? trim( (string) $registrant->class_name ) : '';
        ...
        private function outputCsv( $rows ) {
            ...
            $filename = 'registrants-with-rentals-' . $this->reg_year . '.csv';
            ...
            $headers = array( 'Registration Number', 'Name', 'Address1', 'Address2', 'City', 'State', 'ZIP', 'Country', 'Email', 'Class' );
            fputcsv( $out, $headers );
            ...
            foreach ( $rows as $r ) {
                ...
                $class_name = wp_strip_all_tags( $r->class_name ?? '' );
        ...
        private function getRented() {
            global $wpdb;
            if ( ! class_exists( 'Seminar_Registration_Queries' ) ) {
                return array();
            }
            $sql_template = Seminar_Registration_Queries::get_rented_sql();
...
if ( class_exists( 'Seminar_Rented' ) && ! isset( $seminar_rented ) ) {
    $seminar_rented = new Seminar_Rented();
}
