...
0)valueChanged=false;else inputs[i].selectedIndex=0;var chosenId=inputs[i].id.replace(/[^\w]/g,"_");var autocomplete=document.getElementById(chosenId+"_chosen");if(autocomplete!==null)jQuery(inputs[i]).trigger("chosen:updated")}else inputs[i].value="";if(required==="required"){inputs[i].required=false;inputs[i].setAttribute("aria-required",false)}prevInput=inputs[i]}if(valueChanged===true)triggerChange(jQuery(prevInput))}function isFieldCurrentlyShown(containerId,formId){return isFieldConditionallyHidden(containerId,
formId)===false}function isFieldConditionallyHidden(containerId,formId){var hidden=false,hiddenFields=getHiddenFields(formId);if(hiddenFields.indexOf(containerId)>-1)hidden=true;return hidden}function clearHideFields(){var hideFieldInputs=document.querySelectorAll('[id^="frm_hide_fields_"]');clearValueForInputs(hideFieldInputs)}function addToHideFields(htmlFieldId,formId){var hiddenFields=getHiddenFields(formId);if(hiddenFields.indexOf(htmlFieldId)>-1);else{hiddenFields.push(htmlFieldId);hiddenFields=
