...
function get_red_note() {
	if (get_field ( 'show_red_note', 'option' ))
		return '<div class="red">' . get_field ( 'red_note', 'option' ) . '</div>';
}
 ...
 * A companion to wp_get_attachment_image_src which returns only the URL
 * 
 ...
 *        	Limit the character count (null|false for unlimited)
 * @param string $more
 *        	Text to append to trimmed text
 * @return string
 */
...
function get_balance_individual($num_days, $gala, $age, $eefc, $payment, $dvd, $transport) {
	...
	if ($payment == "on-site") {
		if ($age == "child") {
			if ($num_days == 6)
				$balance = round ( round ( $price * 0.25 ) * 1.37 * 5.5 );
			else
				$balance = round ( round ( $price * 0.25 ) * 1.37 ) * $num_days;
		} else if ($age == "student") {
			if ($num_days == 6)
				$balance = round ( round ( $price * 0.75 ) * 1.37 * 5.5 );
			else
				$balance = round ( round ( $price * 0.75 ) * 1.37 ) * $num_days;
		} else if ($age == "adult") {
			if ($num_days == 6)
				$balance = round ( $price * 1.37 * 5.5 );
			else
				$balance = round ( $price * 1.37 ) * $num_days;
		}
		if ($eefc) {
		...
		if ($age == "child") {
			if ($num_days == 6)
				$balance = round ( round ( $price * 0.25 ) * 5.5 );
			else
				$balance = round ( $price * 0.25 ) * $num_days;
		} else if ($age == "student") {
			if ($num_days == 6)
				$balance = round ( round ( $price * 0.75 ) * 5.5 );
			else
				$balance = round ( $price * 0.75 ) * $num_days;
		} else if ($age == "adult") {
			if ($num_days == 6)
				$balance = round ( $price * 5.5 );
			else
				$balance = round ( $price ) * $num_days;
		}
...
function sendRegisterEmail(&$registration) {
	...
	wp_mail ( $email, 'Folk Seminar Plovdiv | Registration #' . $primary->id, $message, $headers );
}
