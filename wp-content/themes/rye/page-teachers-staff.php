...
<h2>
	<span><?php the_title(); ?><?php if (isset ($urlYear) && $urlYear > 2014) echo ' Archive ' . $lookupYear; ?></span>
</h2>
<div class="c12 row">
		...
		<div
			class="person-info<?php if (!$thumbnail): ?> no-photo<?php endif; ?>">
			<div class="person-name"><?php echo get_the_title(); ?></div>
			<div class="person-position"><?php echo getCorrectClassName ($class_name); ?></div>
		</div>
	</a>

	<div class="person-wrapper-content"
		id="person-wrapper-<?php echo $person_id; ?>">
		<div
			class="person-photo<?php if (!$thumbnail): ?> no-photo<?php endif; ?>"><?php if ($thumbnail): ?><img
				src="<?php echo $thumbnail; ?>" /><?php endif; ?></div>
		<div
			class="person-info<?php if (!$thumbnail): ?> no-photo<?php endif; ?>">
			<div class="person-name"><?php echo get_the_title(); ?></div>
		...
		if ($dance_teacher_query->have_posts ()) {
			...
			while ( $dance_teacher_query->have_posts () ) {
				...
				?>
					...
					<a
		class="person-wrapper dance-group fancybox<?php if ($count % 2 == 0) echo ' odd'; ?>"
		rel="dance-teacher-group"
		href="#group-wrapper-<?php echo $person_id; ?>">
		<div class="hover-overlay"></div>
		<div class="person-photo<?php if (!$thumbnail): ?> no-photo<?php endif; ?>" style="<?php if ($thumbnail): ?> background-image: url('<?php echo $thumbnail; ?>');<?php endif; ?>"></div>
		<div
			class="person-info<?php if (!$thumbnail): ?> no-photo<?php endif; ?>">
	...
	<div class="person-wrapper-content"
		id="group-wrapper-<?php echo $person_id; ?>">
		<div
			class="person-photo<?php if (!$thumbnail): ?> no-photo<?php endif; ?>"><?php if ($thumbnail): ?><img
				src="<?php echo $thumbnail; ?>" /><?php endif; ?></div>
		<div
		...
		<div class="person-content">
			...
			?>
				...
				<?php else: ?>
				<p>No dance groups determined yet.</p> 
			<?php endif;
		}
		wp_reset_postdata ();
		wp_reset_query ();
		...
		<?php
			...
			foreach ( $positions as $person_id => $positions_list ) {
				...
				?>
			<a class="person-wrapper coordinator fancybox" rel="program-group"
		href="#person-wrapper-coordinator-<?php echo $person_id; ?>">
		<div class="hover-overlay"></div>
		<div class="person-photo<?php if (!$thumbnail): ?> no-photo<?php endif; ?>" style="<?php if ($thumbnail): ?> background-image: url('<?php echo $thumbnail; ?>');<?php endif; ?>"></div>
		<div
			class="person-info<?php if (!$thumbnail): ?> no-photo<?php endif; ?>">
	...
	<div class="person-wrapper-content"
		id="person-wrapper-coordinator-<?php echo $person_id; ?>">
		<div
			class="person-photo<?php if (!$thumbnail): ?> no-photo<?php endif; ?>"><?php if ($thumbnail): ?><img
				src="<?php echo $thumbnail; ?>" /><?php endif; ?></div>
		<div
...
$previous_years_updated = get_field ( 'previous_years_archive_after_2014_repeater' );
